...
export function updateDependencyItem(dependency) {
    ...
    try {
        ...
        if (dependency.latestVersion) {
            dependencyItem.dataset.latestVersion = dependency.latestVersion;
        }
        ...
        if (updateButton) {
            if (dependency.isLatest) {
                // 如果是最新版本，更新按钮为绿色"最新"状态
                updateButton.classList.add('latest');
                updateButton.textContent = '最新';
                updateButton.title = `当前版本 ${dependency.version} 已是最新`;
            } else {
                // 如果不是最新版本，更新按钮为黄色"更新"状态
                updateButton.classList.remove('latest');
                updateButton.textContent = '更新';
                updateButton.title = `有新版本可用: ${dependency.latestVersion || '未知'}`;
            }
        }
        
        // 更新所有相关的样式类
        dependencyItem.classList.toggle('latest', dependency.isLatest);
        ...
        console.log(`依赖项 ${dependency.name} UI已更新，版本: ${dependency.version}, 最新: ${dependency.isLatest}`);
        return true;
    } catch (error) {
...
async function loadVersionHistory(packageName, currentVersion) {
    const versionList = document.getElementById('version-list');
    versionList.innerHTML = '<div class="version-loading">加载版本历史与最新版本信息中...</div>';
    
    try {
        // 从core-api.js导入相关方法
        const { getPackageVersions, getCurrentVersion } = await import('./core-api.js');
        
        // 并行请求版本历史和当前版本，提高效率
        const [versionHistoryData, currentVersionData] = await Promise.all([
            getPackageVersions(packageName),
            getCurrentVersion(packageName)
        ]);
          // 获取当前最新版本
        if (currentVersionData.success && currentVersionData.version) {
            // 更新UI中显示的当前版本
            const currentVersionElement = document.getElementById('current-version');
            if (currentVersionElement) {
                currentVersionElement.textContent = currentVersionData.version;
                // 更新当前版本变量，用于后续标记版本列表
                currentVersion = currentVersionData.version;
            }
        }
        
        if (!versionHistoryData.success) {
            versionList.innerHTML = `<div class="version-error">加载失败: ${versionHistoryData.message}</div>`;
            return;
        }
        
        if (versionHistoryData.versions.length === 0) {
            versionList.innerHTML = '<div class="version-error">未找到版本历史</div>';
            return;
        }
        
        // 渲染版本列表
        renderVersionList(versionList, versionHistoryData, currentVersion);
    } catch (error) {
